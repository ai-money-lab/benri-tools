...
(crontab -l 2>/dev/null | grep -v 'money-machine' || true) | {
    ...
    echo "# money-machine - X post 1/3 (07:30 JST)"
    echo "30 7 * * * cd $INSTALL_DIR && xvfb-run --auto-servernum $PYTHON_PATH $X_POST_SCRIPT >> $INSTALL_DIR/logs/cron_xpost.log 2>&1"
    echo "# money-machine - X post 2/3 (12:15 JST)"
    echo "15 12 * * * cd $INSTALL_DIR && xvfb-run --auto-servernum $PYTHON_PATH $X_POST_SCRIPT >> $INSTALL_DIR/logs/cron_xpost.log 2>&1"
    echo "# money-machine - X post 3/3 (20:00 JST)"
    echo "0 20 * * * cd $INSTALL_DIR && xvfb-run --auto-servernum $PYTHON_PATH $X_POST_SCRIPT >> $INSTALL_DIR/logs/cron_xpost.log 2>&1"
} | crontab -
echo "  OK"
...
echo "    07:30  - x_daily_post.py (X投稿 1/3)"
echo "    12:15  - x_daily_post.py (X投稿 2/3)"
echo "    20:00  - x_daily_post.py (X投稿 3/3)"
echo ""
echo "  手動テスト:"
